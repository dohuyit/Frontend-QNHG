...
import {
  ...
  useDistricts,
  useWards,
} from "@hooks/admin/useAdministrativeUnits";
...
const CustomerForm = ({
  ...
  const handleFormikChange = (e) => {
    ...
    if (type === "file") {
      ...
      if (name === "city_id" && value !== formik.values.city_id) {
        // So sánh với formik.values.city_id hiện tại
        formik.setFieldValue("district_id", "");
        formik.setFieldValue("ward_id", "");
      }
      // Logic reset ward_id khi district_id thay đổi
      if (name === "district_id" && value !== formik.values.district_id) {
        // So sánh với formik.values.district_id hiện tại
        formik.setFieldValue("ward_id", "");
      }
    }
  };
                ...
                <Col md={12}>
                  <FormGroup>
                    ...
                    <StatusFilterGroup
                      ...
                      onChange={(val) =>
                        formik.setFieldValue("status_customer", val)
                      }
                      className="mt-2"
                    />
                    {hasError("status_customer") && (
                      <div className="invalid-feedback d-block">
                        {getErrorMessage("status_customer")}
                      </div>
                    )}
                  </FormGroup>
                </Col>
