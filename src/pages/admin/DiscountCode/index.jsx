            ...
            {Array.isArray(data) && data.length > 0 ? (
                data.map((item) => (
                    <tr key={item.id ?? item.code}>
                        ...
                        <td>
                            <Badge
                                color={item.is_active || item.status === "active" ? "success" : "secondary"}
                            >
                                {getStatusLabel(item)}
                            </Badge>
                        </td>
...
const CouponIndex = () => {
    ...
    const fetchCoupons = async (page = 1) => {
        ...
        try {
            ...
            if (Array.isArray(res?.data?.data)) {
                ...
                const firstArray = Object.values(res.data.data).find((v) => Array.isArray(v));
    ...
    const handleCouponClick = (coupon) => {
        setNewCoupon({
            ...
            discount_type: coupon.type || coupon.discount_type || "percent",
            discount_value: coupon.value ?? coupon.discount_value ?? "",
            start_date: coupon.start_date || "",
            end_date: coupon.end_date || "",
            usage_limit: coupon.usage_limit ?? "",
            status: coupon.is_active === false ? "inactive" : coupon.status ?? "active",
        });
        setIsEdit(true);
    ...
    return (
        <div className="page-content">
            <Breadcrumbs
                ...
                breadcrumbItem={activeTab === "list" ? "Danh sách mã giảm giá" : "Thùng rác"}
            />
