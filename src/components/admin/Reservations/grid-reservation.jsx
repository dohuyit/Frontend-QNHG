...
const ReservationGrid = ({
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      ...
      let timeValue =
        reservation.reservation_time || reservation.booking_time || "";
      // Nếu có định dạng 12h thì convert sang 24h
      if (
        typeof timeValue === "string" &&
        (timeValue.includes("CH") || timeValue.includes("SA"))
      ) {
      ...
      const combinedDateTime = `${
        reservation.reservation_date || reservation.booking_date || ""
      } ${timeValue}:00`;
      ...
      if (
        ...
        "Không thể hủy đơn đặt bàn vì đã có món trong đơn hàng. Vui lòng kiểm tra lại!"
      ) {
    ...
    try {
      ...
      toast.success(
        isConfirmingReservation
          ? "Đã xác nhận đơn đặt bàn thành công"
          : "Đã cập nhật đơn đặt bàn thành công"
      );
      if (onUpdate) onUpdate();
      setShowEdit(false);
      setSelectedItem(null);
      ...
      if (error.response && error.response.data && error.response.data.errors) {
        ...
        toast.error(
          "Có lỗi xảy ra trong quá trình cập nhật. Vui lòng kiểm tra lại thông tin."
        );
      } else if (
        error.response?.data?.message ===
        "Không thể hủy đơn đặt bàn vì đã có món trong đơn hàng. Vui lòng kiểm tra lại!"
  ...
  const openEditModal = (item) => {
    ...
    const isConfirming =
      ...
      (item.originalStatus === "pending" || !item.originalStatus);
    ...
    setEditForm({
      ...
      reservation_time: extractTime(
        item.reservation_time || item.booking_time || ""
      ),
  ...
  const openChangeLogsModal = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Không tải được lịch sử thay đổi"
      );
      setChangeLogs([]);
    } finally {
        ...
        <ModalHeader
          toggle={() => {
            ...
            setEditForm({
              ...
              customer_phone:
                selectedItem.customer_phone || selectedItem.phone_number || "",
              customer_email:
                selectedItem.customer_email || selectedItem.email || "",
              reservation_date:
                selectedItem.reservation_date ||
                selectedItem.booking_date ||
                "",
              reservation_time: extractTime(
                selectedItem.reservation_time || selectedItem.booking_time || ""
              ),
              number_of_guests: selectedItem.number_of_guests || "",
              table_area_id:
                selectedItem.table_area_id || selectedItem.table_id || "",
                      ...
                      value={
                        ...
                        editForm.reservation_date !== undefined
                          ...
                          : (
                              selectedItem.reservation_date ||
                              selectedItem.booking_date ||
                              (selectedItem.reservation_time &&
                              selectedItem.reservation_time.length >= 10
              ...
              <Row>
                <Col md={6}>
                  <FormGroup>
                    ...
                    {(() => {
                      const currentStatus =
                        editForm.status || selectedItem.status || "pending";
                      let options = [];
                        ...
                        <>
                          ...
                          {selectedItem.status === "completed" && (
                            <div
                              ...
                              style={{ fontSize: 13 }}
                            >
                              Đơn đặt bàn đã hoàn thành, không thể thay đổi
                              trạng thái khác.
                            </div>
                          )}
                          {selectedItem.status === "no_show" && (
                            <div
                              ...
                              style={{ fontSize: 13 }}
                            >
                              Đơn đặt bàn đã chuyển sang trạng thái "Không đến",
                              không thể thay đổi trạng thái khác.
                            </div>
                          )}
                        </>
                      );
        ...
        <ModalBody className="pt-0">
          {selectedItem && (
            <div className="reservation-detail-modal">
              ...
              <div className="d-flex align-items-center mb-3">
                ...
                <div>
                  {(() => {
                    ...
                    const config = statusMap[selectedItem.status] || {
                      color: "secondary",
                      text: "Không xác định",
                    };
                    return (
                      <span
                        className={`badge bg-${config.color} px-3 py-2`}
                        style={{ fontSize: 14 }}
                      >
                        {config.text}
                      </span>
              ...
              {/* Reservation Details */}
              <div className="mb-4">
                <div className="fw-semibold mb-2" style={{ fontSize: 16 }}>
                  Thông tin đặt bàn
                </div>
                <Row className="gy-2">
                  ...
                  <Col md={4} xs={12} className="d-flex align-items-center">
                    ...
                    <span>
                      ...
                      <div className="fw-bold">
                        {selectedItem.number_of_guests} khách
                      </div>
                    </span>
                  </Col>
                </Row>
                {selectedItem.notes || selectedItem.special_requests ? (
                  <div className="mt-3 d-flex align-items-center">
                    <span className="me-2 text-primary">
                      <i className="mdi mdi-note-text-outline"></i>
                    </span>
                    <span>
                      <div className="text-muted" style={{ fontSize: 13 }}>
                        Ghi chú
