...
const OrderDetailModal = ({ isOpen, toggle, order }) => {
  ...
  const subtotal = items.reduce(
    (sum, item) =>
      sum + Number(item.unit_price || item.price) * Number(item.quantity),
    0
  );
  const vat = order.vat || 0;
  const total = order.final_amount || order.total_amount || subtotal;
  const tableNames =
    Array.isArray(order.tables) && order.tables.length > 0
      ? order.tables
          .map((t) =>
            t.table_number ? `${t.table_number}` : t.id ? `${t.id}` : ""
          )
          .join(", ")
      : order.table_id
      ? `${order.table_id}`
            ...
            <Card className="order-info-card border-0 p-0">
              <CardBody className="p-3">
                ...
                <div className="d-flex justify-content-between mb-2">
                  ...
                  <span>
                    {orderTime
                      ? new Date(orderTime).toLocaleString("vi-VN")
                      : "-"}
                  </span>
                </div>
                <div className="d-flex justify-content-between mb-2">
                  <span className="text-muted">Loại đơn:</span>
                  <span>{getOrderTypeInfo(order.order_type)}</span>
                </div>
                <div className="d-flex justify-content-between mb-2">
                ...
                <div className="d-flex justify-content-between mb-2">
                  ...
                  <span>{order.number_of_people || 1}</span>
                </div>
                <div className="d-flex justify-content-between mb-2">
              ...
              <CardBody className="p-3">
                ...
                {items.length > 0 ? (
                  items.map((item, idx) => (
                    <div
                      key={idx}
                      className="order-item-row d-flex justify-content-between"
                    >
                      <span>
                        {item.name || item.dish_id?.name || item.combo_id?.name}
                      </span>
                      <span className="order-detail-price-lg">
                        {formatPriceToVND(item.unit_price || item.price)} x{" "}
                        {item.quantity}
                      </span>
                    </div>
                  ))
                ...
                <hr className="my-3" />
                <div className="order-detail-row d-flex justify-content-between mb-3 order-detail-price-lg">
                  <span>Tạm tính:</span>
                ...
                <div className="order-detail-total d-flex justify-content-between fw-bold">
                  <span>Tổng cộng:</span>
