...
import { getTrashedCombos, restoreCombo } from "@services/admin/comboService";
import { toast } from "react-toastify";

const ROWS_PER_PAGE = 10;

const ListTrashCombo = ({ refreshKey }) => {
                                ...
                                <tr key={combo.id}>
                                    ...
                                    <td>
                                        <strong style={{ color: "#28a745", fontSize: "1.1em" }}>
                                            {combo.selling_price
                                                ? `${Number(combo.selling_price).toLocaleString()} đ`
                                                : "N/A"}
                                        </strong>
                                    </td>
                                    <td>
                                        {combo.items?.length > 0 ? (
                                            combo.items.map(item => item.dish_name || item.name).join(", ")
                                        ) : (
                                            <span className="text-muted fst-italic">Chưa có món</span>
                                        )}
                                    </td>
                                    <td>
                                        <button
